...
class QuizBloc extends Bloc<QuizEvent, QuizState> {
  ...
  void _onLoadQuestions(LoadQuestions event, Emitter<QuizState> emit) {
    ...
    if (questions == null || questions.isEmpty) {
      ...
      emit(QuizLoaded(selectedQuestions, 0, List.filled(5, null)));
    }
  }
  ...
  void _onSubmitAnswer(SubmitAnswer event, Emitter<QuizState> emit) async {
    if (state is QuizLoaded) {
      ...
      answers[currentState.currentQuestionIndex] = event.userChoice ? "True" : "False";

      if (answers.every((a) => a != null)) {
        int score = 0;
        for (int i = 0; i < answers.length; i++) {
          if ((answers[i] == "True" && currentState.questions[i]['isCorrect'] == true) ||
              (answers[i] == "False" && currentState.questions[i]['isCorrect'] == false)) {
            score++;
          }
        }
